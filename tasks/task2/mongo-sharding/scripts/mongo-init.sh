...
# Shard3
docker compose exec -T shard3 mongosh --port 27020 --quiet <<EOF
try {
  rs.initiate({
    _id: "shard3",
    members: [
      { _id: 0, host: "shard3:27020" }
    ]
  });
} catch (e) {
  if (e.codeName !== 'AlreadyInitialized') {
...
until docker compose exec -T shard3 mongosh --quiet --port 27020 --eval "db.hello().isWritablePrimary" | grep true; do sleep 2; done
